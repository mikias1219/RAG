...
const prismaCompany = (prisma as PrismaClient & { company: any }).company;
const prismaWorkspace = (prisma as PrismaClient & { workspace: any }).workspace;
const prismaWorkspaceMember = (prisma as PrismaClient & { workspaceMember: any }).workspaceMember;
...
async function main() {
  ...
  await prismaWorkspace.deleteMany();
  await prismaCompany.deleteMany();
  await prismaUser.deleteMany();

  const tenantId = "t_default";

  const company = await prismaCompany.create({
    data: {
      tenantId,
  ...
  const workspace = await prismaWorkspace.create({
    data: {
      ...
      industry: "general",
      domainFocus: "General enterprise knowledge operations"
    }
  });
  ...
  await prismaWorkspaceMember.createMany({
    data: [
      { workspaceId: workspace.id, userId: "u_superadmin", role: "superadmin", status: "active" },
      { workspaceId: workspace.id, userId: "u_admin", role: "admin", status: "active" },
      { workspaceId: workspace.id, userId: "u_default", role: "user", status: "active" }
    ]
  });
